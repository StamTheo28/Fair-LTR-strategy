    ...
    def transform(self, input):   
        ...
        for index, row in input.iterrows():
            if count%10000==0:
                print(count)
            docid = row['docid']
            features = get_var_4_feature_list()
            f_scores = data[get_var_4_feature_list()][data.docid == docid]
            f_list = []
            for f in features:
                if len(f_scores[f]) == 0:
                    f_list.append(0.0)
...
def all_features_var():
    var_4_path = 'data-models/Data/var_4.pkl'
    vardf_4_path = 'data-models/Data/vardf_4.pkl'

    if not os.path.exists(vardf_4_path):
        if not os.path.exists(var_4_path):
            print('Global dataset statistics not available for variation 4 ')
            ...
            f = open(var_4_path,"wb")
            # write the python object (dict) to pickle file
            pickle.dump(var_dict,f)

            # close file
            f.close()
            print('Variation 4 dictionary created and saved!')
            del pop_stats
            del df
            gc.collect()
        else:    
            var_dict = pd.read_pickle(var_4_path)   
            print('Dictionary has been imported')

        chunk_size = 10000  
        vardf_4_path = 'data-models/Data/vardf_4.pkl'
        
        with pd.HDFStore(vardf_4_path, mode='w') as store:
        ...
        del var_dict
        gc.collect()
        var_df = pd.read_hdf(vardf_4_path, key='df')
        print('Variation 4 scores reated and loaded')

    else:
        var_df = pd.read_hdf(vardf_4_path, key='df')
        print('Variation 4 scores loaded')
    return var_df
